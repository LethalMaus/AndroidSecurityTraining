...
fun RisksScreen() {
    ...
    fun showAppConfig(): String {
        ...
        val ai: ApplicationInfo = pm.getApplicationInfo(context.packageName, 0)
        ...
        val windowSecure = (context as? Activity)?.window?.let { (it.attributes.flags and WindowManager.LayoutParams.FLAG_SECURE) != 0 } ?: false
        return buildString {
    ...
    fun listExported(): String {
        ...
        val pi: PackageInfo = pm.getPackageInfo(context.packageName, flags)
    ...
    Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
        Column(
            ...
            Button(onClick = {
                output.value = showAppConfig()
            }) { Text("Show App Config (debuggable, backup, cleartext, FLAG_SECURE)") }
            Button(onClick = {
                scope.launch {
                    output.value = testCleartext()
                }
            }) { Text("Test Cleartext HTTP (neverssl.com)") }
            Button(onClick = { output.value = listExported() }) { Text("List Exported Components") }
            Spacer(Modifier.height(8.dp))
            Button(onClick = {
                if (context is Activity) {
                    val msg = provideRisksHelper().toggleFlagSecure(context.window)
                    output.value = msg + "\n\n" + showAppConfig()
                } else {
                    output.value =
                        "Not an Activity context; cannot toggle FLAG_SECURE in preview.\n\n" + showAppConfig()
                }
